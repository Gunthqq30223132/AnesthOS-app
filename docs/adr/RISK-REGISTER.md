...
> kế hoạch đáp ứng. Cập nhật qua PR như mọi tài liệu khác. Trạng thái: 2026-07-21 (A2 án lệ gray-market proxy; A5 M1-11 đóng + nợ M1-14 combo).
...
| A2 | Hack OAuth của Kiro (API key giả dạng oauth, `expiresAt: 2030`) vỡ khi 9router/Kiro đổi schema → kênh Lính-mạnh chết im lặng. **Án lệ 2026-07-21:** khi kết nối API Kiro chính thức trả 403, Chủ được mời cài "kiro.pix4k.com" — proxy gray-market bên thứ ba (không phải Amazon/Anthropic), hướng dẫn dùng `curl\|bash` từ domain lạ + override `ANTHROPIC_BASE_URL` + tắt `CLAUDE_CODE_DISABLE_REFUSAL_FALLBACK`. TỪ CHỐI cài — 403 là triệu chứng của gray-market/hack OAuth vỡ (đúng kịch bản A2), không phải bug sửa-tại-máy; cài proxy lạ = RCE qua curl\|bash + rò `ANTHROPIC_BASE_URL` + xoá luôn provenance (không còn biết leaf thật nào phục vụ) | C/C | Mitigate: smoke test model pin trong preflight mỗi phiên làm việc; Accept phần còn lại (hack là tạm). Fallback: Antigravity tự code + PM audit dày. **Avoid (mới):** không bao giờ cài/dùng proxy gray-market thay Kiro/Anthropic thật cho hệ lâm sàng; 403 → sửa bằng kênh chính chủ (Kiro/Anthropic/AWS Bedrock) hoặc route qua model khác đã verified, không đổi `ANTHROPIC_BASE_URL` sang domain không kiểm chứng | Antigravity + Chủ | 401/403/`expired` trong log 9router; latency bất thường; **bất kỳ hướng dẫn cài đặt nào yêu cầu đổi `ANTHROPIC_BASE_URL`/`ANTHROPIC_API_KEY` sang domain ngoài danh sách nhà cung cấp đã biết (Anthropic, AWS Bedrock, Kiro chính chủ)** |
...
| A5 | **[M1-11 ĐẠT — nợ mới M1-14]** 9router có combo `claude-sonnet-4.5` (Fusion 6-model) TRÙNG TÊN GỐC với leaf `kr/claude-sonnet-4.5`, chỉ khác prefix. `response.model` bị cắt prefix → `model_returned` một mình không phân biệt leaf/combo. M1-11 đã đóng (runner ghi `req_model` có-prefix, verifier kiểm ∈ Verified). **2026-07-21:** Luật Combo/Alias đổi — cho phép Fallback/Capacity-switch combo (tên riêng, mọi thành viên verified) cho dispatch Tier-2 xuống-dưới (code-gen, không PHI); guarantee chuyển từ `req_model` có-prefix sang **`model_returned` (leaf thật đã phục vụ) ∈ VERIFIED[tier]** — vì TARGET giờ hợp lệ là tên combo không-prefix. Verifier CHƯA cập nhật check này (nợ M1-14); cổng CI M1-13.1 (dotenv crash) cũng chưa xác nhận xanh thật | C/C | Done: leaf quan sát được qua `provider`+`req_model` (M1-10); M1-11 đóng. Nợ (M1-14): `dispatch_verifier.py` chuyển hard-check chính sang `model_returned ∈ VERIFIED_MODELS[tier]`, không hardcode; Fallback/Capacity combo CHỈ dùng khi MỌI thành viên đã smoke-verify riêng TRƯỚC khi ghép combo. Round-Robin/Fusion vẫn CẤM sinh-patch; cấm combo trùng tên model (Án lệ 1) | Antigravity → PM | `model_returned` == tên trần không phân giải được leaf (combo/alias mù); TARGET là combo Round-Robin/Fusion; thành viên combo chưa có trong `VERIFIED_MODELS` mà đã bật dùng cho dispatch |
