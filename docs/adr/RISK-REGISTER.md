...
| A5 | **[XÁC MINH XONG — M1-10, còn nợ wiring M1-11]** 9router có combo `claude-sonnet-4.5` (Fusion 6-model) TRÙNG TÊN GỐC với leaf `kr/claude-sonnet-4.5`, chỉ khác prefix. `response.model` bị cắt prefix → `model_returned` một mình không phân biệt leaf/combo. First Light ĐÃ xác nhận chạy leaf thật (SQLite rowid 2021, provider=kiro, tokens 8787/3148 khớp biên lai) NHƯNG gate tự động chưa tự đứng được | C/C | Done: leaf quan sát được qua `provider`+`req_model` (M1-10). Nợ (M1-11): runner ghi `provider`+`req_model` đủ prefix; verifier kiểm ∈ Verified; TARGET cấm tên trần không prefix (§4/§14D.4) | Antigravity → PM | `model_returned` == tên trần không prefix; TARGET rớt `kr/`; dispatch tới `owned_by: combo` |
