...
4. **Kiểm provenance trên LEAF thật (bổ sung 2026-07-20; hiệu chỉnh sau M1-10):** verifier phải xác định leaf model thật đã chạy và leaf đó ∈ Verified[tier]. **Bài học M1-10:** `response.model` (nguồn của `model_returned`) bị 9router **cắt prefix provider**, nên nó KHÔNG phân biệt được leaf `kr/X` với combo cùng tên `X` — verify chỉ bằng field này là bất khả thi. Nguồn phân giải đúng: `provider` + `req_model` đầy đủ prefix (9router lộ qua SQLite `requestDetails` và trong error payload `[provider/model]`). **Runner phải ghi thêm `provider` + `req_model` (đủ prefix) vào biên lai; verifier kiểm `provider/req_model` ∈ Verified[tier].** Combo (owned_by combo) hoặc tên trần không prefix → UNVERIFIABLE → FAIL. *(Trạng thái 2026-07-21: M1-11 ĐÃ wire — runner ghi `provider`+`req_model`, verifier bắt prefix + `∈ VERIFIED_MODELS` + chặn tên trần; 23 test liên quan PM tự chạy PASS. Nợ còn lại: `VERIFIED_MODELS` hardcode trong `dispatch_verifier.py` phải giữ đồng bộ tay với bảng §4 — chưa một-nguồn-chân-lý.)*
